...
function TapeEditor({ onNotice, onConnectMidi, onSendMidi }: { onNotice: (message: string) => void; onConnectMidi: () => Promise<boolean>; onSendMidi: (data: number[]) => void }) {
  ...
  return (
    <div className="tool-body tape-editor">
      ...
      <StudioTrackList Icon={Icon} tracks={tracks} files={files} sources={sources} muted={muted} solo={solo} playing={playing} audioRefs={audioRefs} onFileLoad={(index, file) => { setFiles({ ...files, [index]: file.name }); setSources({ ...sources, [index]: URL.createObjectURL(file) }); setDurations({ ...durations, [index]: 0 }); onNotice(`${tracks[index]} chargée localement.`); }} onTogglePlay={togglePlay} onSoloChange={(index) => setSolo(solo === index ? null : index)} onMuteChange={(index) => setMuted({ ...muted, [index]: !muted[index] })} onDurationChange={(index, duration) => setDurations((current) => ({ ...current, [index]: duration }))} onTrackEnd={() => { if (transportPlaying) setTransportPlaying(false); setPlaying(null); }} />

...
export function SoundControlsPanel({ Icon }: { Icon: SoundIcon }) {
  ...
  return (
    <section className="sound-control-panel" aria-labelledby="sound-controls-title">
      <div className="mod-section-heading"><div><span className="section-label">PREPARATION PATCH</span><strong id="sound-controls-title">Controles audio</strong></div><small>{mode === "synth" ? "1 WAV - 6 s max" : `${lowRes ? "24" : "12"} s max - 24 touches`}</small></div>
      <div className="sound-mode-switch" role="group" aria-label="Mode de patch">
        <button type="button" className={mode === "synth" ? "is-active" : ""} onClick={() => setMode("synth")}><Icon name="wave" size={15} />Synthe</button>
      ...
      <div className="sound-tool-status"><span><i /> Moteur de patch detecte</span><code>op-patch-util 1.1.0</code></div>
    </section>
  );
}

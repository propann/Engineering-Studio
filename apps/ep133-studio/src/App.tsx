...
import { archiveStudioProject, deleteStudioProject, duplicateStudioProject, loadStudioLibrary, renameStudioProject, restoreStudioProject, storeStudioProject, studioStateFromDocument, summarizeStudioProject, type StudioProjectRecord, type StudioProjectState } from './core/project/studioLibrary';
...
import { collectLocalFiles, listStudioProjectFileIds, removeStudioProjectFile, writeStudioProjectFile, type LocalDirectoryHandle } from './core/storage/localFolders';
...
function loadHubMachine() {
  const params = new URLSearchParams(window.location.search);
  const queryName = params.get('hubMachineName');
  const queryCapacity = params.get('hubMachineCapacityMb');
  if (queryName || queryCapacity) return { name: queryName || 'EP-133 K.O. II', capacityMb: queryCapacity === '128' ? 128 as const : 64 as const };
  try {
    const stored = JSON.parse(localStorage.getItem(IMPORTED_MACHINE_KEY) || '{}') as { name?: string; capacityMb?: 64 | 128 };
    return { name: stored.name || 'EP-133 K.O. II', capacityMb: stored.capacityMb === 128 ? 128 as const : 64 as const };
  } catch { return { name: 'EP-133 K.O. II', capacityMb: 64 as const }; }
}
...
export default function App() {
  ...
  // Bibliothèque de sons personnelle, dont l’espace de travail sera fourni
  // par le Studio Hub ; aucun profil joueur n’est réglé dans EP‑133.
  ...
  /** Restauration silencieuse du dossier machine mémorisé — queryPermission seul. */
  useEffect(() => {
    void (async () => {
      ...
      if (await hasStoredPermission(handle, 'read')) {
        try { setMachineSampleCount(await machineSampleBank.load(await collectLocalFiles(handle))); } catch { /* permission à redemander depuis le Hub */ }
      }
    })();
  }, []);

  /** Le Hub est l'unique configurateur du workspace. Le handle transmis par
   * postMessage est réduit à la zone EP-133 avant toute écriture locale. */
  useEffect(() => {
    const onHubWorkspace = (event: Event) => {
      const root = (event as CustomEvent<LocalDirectoryHandle | null>).detail;
      if (!root) return;
      void (async () => {
        try {
        ...
        } catch { /* le Hub pourra renvoyer le handle après reconnexion */ }
      })();
    };
  ...
  /** Le dossier vient du Hub ; cette action ne peut plus ouvrir un autre sélecteur. */
  ...
  /** Miroir best-effort de la bibliothèque Studio dans le workspace Hub. */
  ...
  /** Même principe que le workspace Hub, appliqué à toute la
  ...
  const restoreSelectedStudioProject = () => {
    if (!selectedStudioProject) return;
    updateStudioLibrary(restoreStudioProject(localStorage, studioLibrary, selectedStudioProject));
  };
  ...
  return <main className={last ? `impact impact-${last.grade.toLowerCase()}` : ''}>
    ...
    {editorOpen && <div className="editor-overlay"><section className="exercise-editor">
      ...
      {machineCloneOpen && <MachineCloneDialog inventory={deviceInventory} soundIndex={deviceSoundIndex} workspaceDirectory={sampleDirectoryHandleRef.current} onClose={() => setMachineCloneOpen(false)} />}
    </section></div>}

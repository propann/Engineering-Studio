...
/**
 * Écrit un projet Studio dans `<dossier de travail>/projects/<id>.ep.project.json`
...
export async function writeStudioProjectFile(parent: LocalDirectoryHandle, id: string, document: object) {
  const studio = await parent.getDirectoryHandle('projects', { create: true });
...
export async function removeStudioProjectFile(parent: LocalDirectoryHandle, id: string) {
  try {
    const studio = await parent.getDirectoryHandle('projects');
    await studio.removeEntry(`${id}${STUDIO_PROJECT_EXT}`);
  } catch {
    // Déjà absent (dossier projects/ jamais créé, ou fichier déjà supprimé) : rien à faire.
  }
}

/** Ids des projets Studio déjà miroités dans `<dossier de travail>/projects/` — sert à repérer les fichiers à supprimer lors de la réconciliation. */

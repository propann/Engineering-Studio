...
import { createDeviceClone, loadDeviceClone, type DeviceCloneManifest, type DeviceProfile } from '../../core/project/deviceProfile';
import { writeCloneManifest, type LocalDirectoryHandle } from '../../core/storage/localFolders';

interface MachineCloneDialogProps {
  inventory: DeviceInventory | null;
  soundIndex: DeviceSoundIndex | null;
  workspaceDirectory: LocalDirectoryHandle | null;
  onClose: () => void;
}

export function MachineCloneDialog({ inventory, soundIndex, workspaceDirectory, onClose }: MachineCloneDialogProps) {
  const [name, setName] = useState('MON EP-133');
  const [capacityMb, setCapacityMb] = useState<64 | 128>(64);
  const [folderName, setFolderName] = useState(workspaceDirectory?.name || '');
  const [created, setCreated] = useState(false);
  const [directory, setDirectory] = useState<LocalDirectoryHandle | null>(workspaceDirectory);
  ...
  const chooseFolder = async () => {
    try {
      if (!workspaceDirectory) { window.alert('Ouvre ce studio depuis le Studio Hub pour connecter son workspace.'); return; }
      setDirectory(workspaceDirectory); setFolderName(workspaceDirectory.name);
    } catch (error) { window.alert(error instanceof Error ? error.message : 'Impossible d’utiliser le workspace Hub.'); }
  };
  const createClone = async () => {
    if (!bridgeRoot && !directory) { setCloneError('Workspace Hub absent : ouvre le studio depuis le portail Hub.'); return; }
    setCloneError('');
    try {
      const profile: DeviceProfile = { name: name.trim() || 'MON EP-133', capacityMb, sampleFolderName: folderName, localSampleCount: 0, updatedAt: new Date().toISOString() };

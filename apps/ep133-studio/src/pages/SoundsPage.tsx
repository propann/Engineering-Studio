...
interface SoundsPageProps {
  ...
  /** Bibliothèque perso fournie par l’espace de travail du Studio Hub. */
...
export function SoundsPage({ machineName, machineCapacityMb, inventory, soundIndex, midiConnected, machineGroup, onMachineGroupChange, liveMidi, padModes, onBack, onConnectMidi, onPadModeChange, onPadPreview, onPreviewSound, localLibraryHandle, localLibraryFolderName, localLibraryNeedsReconnect, onReconnectLocalLibrary, demoProjects, localProjects, onGetProjectDocument, onImportMachineProject }: SoundsPageProps) {
  // L’identité et la capacité viennent de la fiche Hub importée en lecture seule.
  const capacityMb = machineCapacityMb;
  const deviceName = machineName;
  ...
  // Recharge la racine dès que le dossier fourni par le Hub devient disponible.
  useEffect(() => {
  ...
  return <main className="sound-library-page sound-machine-page">
    ...
    <section className="sound-machine-console">
      <div className="sound-machine-topline"><div><small>MACHINE</small><strong>{deviceName}</strong></div><div className="sound-memory"><span><b>{usedMb.toFixed(2)} MO</b> / {capacityMb} MO · THÉORIQUE <b>{usedMb.toFixed(2)} MO</b></span><i><span style={{ width: `${usedPercent}%` }} /></i><small>{changeCount ? `${changeCount} AFFECTATION(S) · +0 OCTET` : 'AUCUN CHANGEMENT PRÉPARÉ'}</small></div><button onClick={onConnectMidi}>{midiConnected ? 'EP-133 CONNECTÉ ✓' : 'CONNECTER EP-133'}</button><button className={`sound-sync ${changeCount ? 'active' : ''}`} disabled={!changeCount || syncing} onClick={() => void requestSync()}>{syncing ? 'ENVOI…' : `SYNCHRONISER · ${changeCount}`}</button></div>
      ...
      <div className="sound-machine-workspace">
        ...
        <section className="sound-bank-panel">
          ...
          <div className="sound-bank-browser">
            ...
            <div className="sound-bank-results"><label>RECHERCHER<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="SLOT OU NOM" /></label><div>{filteredSounds.map((sound) => { const bank = bankForSlot(sound.slot); const changed = changedSlots.has(sound.slot); const proposedImport = stagedImports[sound.slot]; return <article key={sound.slot} draggable
              ...
              <div><strong>{namesBySlot.get(sound.slot) || sound.fileName.replace(/\.pcm$/i, '')}</strong><small>{previewMissSlot === sound.slot ? 'AUCUN AUDIO LOCAL — vérifie l’espace de travail du Studio Hub' : proposedImport ? `SON PERSO PROPOSÉ · ${proposedImport.fileName}` : `${bank.label} · ${(sound.bytes / 1000).toFixed(1)} KO · GLISSER SUR UN PAD OU ICI`}</small></div>
        ...
        <section className="sound-bank-panel local-library-panel">
          ...
          {!localLibraryHandle && <p className="local-library-hint">La bibliothèque personnelle sera fournie par l’espace de travail du Studio Hub.</p>}

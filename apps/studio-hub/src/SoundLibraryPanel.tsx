...
export function SoundLibraryPanel({ workspaceHandle = null, onOpenOp1 = () => {}, onOpenEp133 = () => {} }: { workspaceHandle?: FileSystemDirectoryHandle | null; onOpenOp1?: () => void; onOpenEp133?: () => void }) {
  ...
  function scanForDuplicates() {
    ...
    setStatus(`✅ Scan terminé: ${dups.size} groupe${dups.size > 1 ? "s" : ""} de doublons trouvé${dups.size > 1 ? "s" : ""}`);
  }

  async function removeDuplicate(asset: SoundLibraryAsset) {
    if (!window.confirm(`Supprimer le doublon « ${asset.name} » ?`)) return;
    await removeAsset(asset);
    scanForDuplicates(); // Rescan after removal
  }

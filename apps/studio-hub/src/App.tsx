...
const toolLaunches: ToolLaunch[] = [
  ...
  { id: "ep133-sounds", machine: "ep133", tool: "sounds", title: "Sons & transferts EP‑133", category: "SON", description: "Bibliothèque de sons, réglages de pads et préparation des transferts.", icon: "🔊", color: "pink", action: "Ouvrir les sons" },
...
export function App() {
  ...
  return <Dashboard profile={profile} workspaceHandle={workspaceHandle} onProfile={() => setScreen("profile")} onWorkspaceSelected={(handle, name) => { setWorkspaceHandle(handle); setProfile((current) => current ? { ...current, workspace: { name, folders: ["shared", "op1/backups", "op1/samples", "op1/firmware", "ep133/backups", "ep133/projects", "ep133/samples"] } } : current); void saveWorkspaceHandle(handle); }} onBackupRecorded={(machine) => setProfile((current) => current ? { ...current, machines: { ...current.machines, [machine]: { ...current.machines[machine], backups: current.machines[machine].backups + 1 } } } : current)} onStatsReceived={(machineId, stats) => setProfile((current) => {

...
type ToolLaunch = { id: string; machine: MachineId; tool?: "editor" | "sounds" | "services" | "game"; title: string; category: string; description: string; icon: string; color: string; action: string; anchor?: string };

const toolLaunches: ToolLaunch[] = [
  { id: "op1", machine: "op1", title: "OP‑1 Studio", category: "MACHINE", description: "Tape, sons, firmware, sauvegardes et outils créatifs OP‑1.", icon: "🎛️", color: "blue", action: "Ouvrir OP‑1" },
  { id: "ep133", machine: "ep133", title: "EP‑133 Studio", category: "MACHINE", description: "Patterns, Song, samples, clone, MIDI et entraînement.", icon: "🥁", color: "orange", action: "Ouvrir EP‑133" },
  { id: "op1-images", machine: "op1", tool: "editor", title: "Éditeur d’image", category: "CRÉATION", description: "Préparer les écrans et patches graphiques OP‑1 aux bonnes dimensions.", icon: "🖼️", color: "purple", action: "Ouvrir l’éditeur" },
  { id: "op1-samples", machine: "op1", tool: "sounds", title: "Éditeur de samples", category: "SON", description: "Préparer, convertir et organiser les samples avant transfert vers l’OP‑1.", icon: "🌊", color: "teal", action: "Ouvrir les samples" },
  { id: "op1-services", machine: "op1", tool: "services", title: "Services OP‑1", category: "ATELIER", description: "Firmware, patchs son, catalogue de ressources et outils de préparation.", icon: "🧰", color: "yellow", action: "Ouvrir les services" },
  { id: "ep133-sounds", machine: "ep133", tool: "sounds", title: "Sons & transferts EP‑133", category: "SON", description: "Réglages de pads et préparation des transferts audio vers l’EP‑133.", icon: "🔊", color: "pink", action: "Ouvrir les sons" },
  { id: "ep133-game", machine: "ep133", tool: "game", title: "Jeux & entraînement", category: "PRATIQUE", description: "RHYTHM HERO et modules de pratique liés à l’EP‑133.", icon: "🎮", color: "green", action: "Ouvrir les jeux" },
  { id: "midi-sync", machine: "op1", title: "Synchronisation MIDI", category: "TRANSPORT", description: "Jouer l’OP‑1 et l’EP‑133 ensemble avec un tempo, un démarrage et un arrêt communs.", icon: "🔗", color: "indigo", action: "Ouvrir la synchronisation", anchor: "midi-sync" },
  { id: "sound-library", machine: "op1", title: "Bibliothèque de sons", category: "RESSOURCE", description: "Catalogue local commun, tags, favoris, préécoute et préparation pour les deux machines.", icon: "🎵", color: "teal", action: "Gérer les sons", anchor: "sound-library" },
];
...
async function chooseWorkspace(): Promise<WorkspaceSelection | null> {
  ...
  const folders = ["shared", "shared/sounds", "shared/sounds/originals", "shared/sounds/prepared", "shared/sounds/packs", "shared/sounds/quarantine", "op1/backups", "op1/samples", "op1/firmware", "ep133/backups", "ep133/projects", "ep133/samples"];
...
export function App() {
  ...
  return <Dashboard profile={profile} workspaceHandle={workspaceHandle} onProfile={() => setScreen("profile")} onWorkspaceSelected={(handle, name) => { setWorkspaceHandle(handle); setProfile((current) => current ? { ...current, workspace: { name, folders: ["shared", "shared/sounds", "shared/sounds/originals", "shared/sounds/prepared", "shared/sounds/packs", "shared/sounds/quarantine", "op1/backups", "op1/samples", "op1/firmware", "ep133/backups", "ep133/projects", "ep133/samples"] } } : current); void saveWorkspaceHandle(handle); }} onBackupRecorded={(machine) => setProfile((current) => current ? { ...current, machines: { ...current.machines, [machine]: { ...current.machines[machine], backups: current.machines[machine].backups + 1 } } } : current)} onStatsReceived={(machineId, stats) => setProfile((current) => {
...
function Dashboard({ profile, workspaceHandle, onProfile, onWorkspaceSelected, onBackupRecorded, onStatsReceived }: { profile: HubProfile; workspaceHandle: FileSystemDirectoryHandle | null; onProfile: () => void; onWorkspaceSelected: (handle: FileSystemDirectoryHandle, name: string) => void; onBackupRecorded: (machine: MachineId) => void; onStatsReceived: (machine: MachineId, stats: StudioStats) => void }) {
  ...
  const studioWindows = useRef(new Map<Window, string>());
  ...
  function openStudio(id: MachineId, tool?: ToolLaunch["tool"]) {
    ...
    if (child) studioWindows.current.set(child, new URL(machineCopy[id].url).origin);

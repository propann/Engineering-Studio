...
/**
 * Les reglages par defaut des vingt moteurs.
 *
 * Repris des etats initiaux du rack, pour qu'un moteur joue PAREIL partout :
 * le meme `mi_plaits` au clavier de l'OP-1, dans un motif Strudel et dans le
 * rack, tant qu'on n'a rien touche.
 *
 * Un appelant qui n'a pas d'interface de reglage — le clavier en mode
 * controleur, un motif Strudel — part de la et surcharge ce dont il a besoin.
 * Sans cette table, chacun inventerait ses valeurs et le meme nom de moteur
 * donnerait trois sons differents.
 */
export const PARAMS_DEFAUT: ParamsMoteurs = {
  acidAccent: true,
  acidCutoff: 2400,
  acidDecay: 65,
  acidEnvMod: 80,
  ...
  braidsBitDepth: 16,
  braidsColor: 70,
  braidsModel: "CS-80 SAW",
  braidsTimbre: 85,
  cloudsFeedback: 65,
  cloudsGranularDensity: 90,
  cloudsPitchShift: 7,
  cloudsPosition: 50,
  cloudsReverb: 80,
  cloudsTexture: 80,
  ...
  fluidChorus: 30,
  fluidPan: 50,
  fluidPreset: "Acoustic Grand Piano",
  fluidReverb: 60,
  fluidVolume: 90,
  helmCrossmod: 60,
  helmCutoff: 2800,

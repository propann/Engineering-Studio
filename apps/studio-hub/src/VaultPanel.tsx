...
export function VaultPanel({
  ...
  /**
   * Nom de la machine pour les libelles.
   *
   * L'EP-133 n'a AUCUN mode disque : ses sons passent par SysEx. Parler de
   * « disque EP-133 » serait faux, et la regle produit du projet l'interdit
   * explicitement — l'interface ne doit jamais laisser croire qu'une machine a
   * ete lue quand on n'a fait qu'inspecter un dossier.
   */
  const nomMachine = machine === "op1" ? "OP‑1" : "EP‑133";
  const supportMachine = machine === "op1" ? "Disque OP‑1" : "Dossier EP‑133";
...
async function createBackup() {
  ...
  return <section className={`vault-panel ${compact ? "vault-panel-compact" : ""}`} id={`atelier-vault-${machine}`}>
    <div className="vault-header"><div><span className="section-kicker">COFFRE {machine === "op1" ? "OP‑1" : "EP‑133"}</span><h2>Gérer les sauvegardes</h2><p className="muted">Coche ce que tu veux copier. Tout reste sur ton ordinateur, rien n'est envoyé ailleurs.</p></div>{showWorkspace && (
    ...
    <div className="vault-grid">
      <div className="vault-card"><div className="vault-card-heading"><div><span className="section-kicker">1 · COPIER DEPUIS LA MACHINE</span><h3>Copier {nomMachine} sur cet ordinateur</h3></div><button type="button" className="secondary-button" onClick={() => void chooseSource()}>{source ? "Relancer le scan" : `Choisir le ${supportMachine}`}</button></div><p className="vault-warning">⏱️ Une sauvegarde peut prendre plusieurs minutes. Ne débranche pas la machine et garde cette fenêtre ouverte.</p><p className="vault-path">{source ? `Depuis : ${sourceName}` : `Choisis le ${supportMachine} à copier`}</p>{sourceScan && (

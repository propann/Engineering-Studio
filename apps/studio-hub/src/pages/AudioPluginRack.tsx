...
export default function AudioPluginRack({
  ...
  const construireVoix = (
    ...
    /**
     * Le moteur actif, construit par la bibliotheque partagee.
     *
     * Les 848 lignes de `else if` qui vivaient ici sont dans
     * `core/audio/moteurs.ts` depuis le 2026-08-29. Tant qu'elles etaient dans
     * ce composant, un seul ecran pouvait les jouer : le clavier de l'OP-1
     * n'en atteignait que sept via une reimplementation partielle, les outils
     * d'echantillon aucun, et Strudel ne pouvait pas les appeler.
     *
     * La chaine rend une description au lieu d'afficher un toast : un moteur
     * n'a pas a connaitre l'interface qui l'utilise.
     */
    const detunedFreq = freq * Math.pow(2, p.masterDetune / 1200);
    const message = construireMoteur(
      ctx,
      p,
      detunedFreq,
      now,
      { trk, noteStop, holdUntil, reverb: reverbPour(ctx) },
      masterGain,
    );

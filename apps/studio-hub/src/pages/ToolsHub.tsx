...
export default function ToolsHub(){
 ...
 return <main className="hub-page">
  ...
  <section className="machine-studios" id="studios"><div className="machine-studio-grid">
   <button className="machine-card op1-card" onClick={()=>openTool(machineTool("op1"))}><div className="machine-card-image"><img src="/media/teenage-engineering-op-1-pixel-art-music-workstation.jpeg" alt="Teenage Engineering OP-1 complet en pixel art"/><div className="machine-online"><i/> LOCAL MODE</div></div><div className="machine-card-copy"><span>SYNTHÈSE · TAPE · FIRMWARE</span><h3>OP-1 Studio</h3><p>Firmware, sauvegardes, sons, Tape, images, services, exercices et documentation.</p><div className="module-pills"><b>8 MODULES</b><b>MIDI</b><b>LOCAL</b></div></div></button>
   <button className="machine-card ko-card" onClick={()=>openTool(machineTool("ep133"))}><div className="machine-card-image"><img src="/media/teenage-engineering-ep-133-ko-ii-pixel-art-sampler.jpeg" alt="Teenage Engineering EP-133 K.O. II complet en pixel art"/><div className="machine-online"><i/> 64 MO · PRÊT</div></div><div className="machine-card-copy"><span>BEATS · SONG · PERFORMANCE</span><h3>EP-133 Studio</h3><p>Pattern & Song, sons, test machine, Rhythm Hero et documentation.</p><div className="module-pills"><b>5 MODULES</b><b>SYSEX</b><b>PADS</b></div></div></button>
   <button className="machine-editor-card sound-library-card" onClick={()=>openTool(hubTool("library"))}><div className="subtool-wave">{[32,68,45,88,56,96,40,76,50,82].map((v,i)=><i key={i} style={{height:`${v}%`}}/>)}</div><div><b>SOUND-V1 · CONTENU AUDIO</b><h3>Bibliothèque de samples</h3><p>Importer, classer, écouter et préparer les banques de sons OP-1.</p></div><strong>↗</strong></button>
  </div></section>

  <section className="utility-cards" aria-label="Accès rapides"><div className="utility-grid">
   <button className="utility-card purple" onClick={()=>openTool(tools.find(t=>t.id==="op1-docs")!)}><span>DOC-01</span><ToolGraphic type="pixels"/><small>RÉFÉRENCES VÉRIFIÉES</small><h3>Documentation</h3><p>Guides OP-1 et EP-133, formats, MIDI et procédures.</p></button>
   <button className="utility-card orange" onClick={()=>openTool(tools.find(t=>t.id==="image")!)}><span>PX-320</span><ToolGraphic type="pixels"/><small>CRÉATION VISUELLE</small><h3>Éditeur d’image</h3><p>Écrans, thèmes et visuels au format exact des machines.</p></button>
   <button className="utility-card yellow" onClick={()=>openTool(hubTool("vault"))}><span>VAULT-01</span><ToolGraphic type="chip"/><small>COFFRE LOCAL</small><h3>Sauvegarde</h3><p>Snapshots OP-1 et EP-133 avec contrôle d’intégrité.</p></button>
  </div></section>

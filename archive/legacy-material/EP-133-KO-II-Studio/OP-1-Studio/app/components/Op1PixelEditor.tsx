...
export function Op1PixelEditor({ width, height, sourceSvg, filename = "op1-image", onNotice }: { width: number; height: number; sourceSvg: string; filename?: string; onNotice?: (message: string) => void }) {
  ...
  useEffect(() => {
    const updatePixelsFromSvg = async () => {
      if (!supported || !sourceSvg) {
        setPixels(blankPixels(width, height));
        return;
      }
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        image.onerror = () => reject(new Error("SVG refusé : impossible de le rasteriser dans les dimensions déclarées."));
        image.src = url;
